...
export default function HomePage() {
  ...
  const handleAnalyze = useCallback((tree: string) => {
    setIsLoading(true)
    // Small async tick to let the loading state render
    setTimeout(() => {
      try {
    ...
    <div className="min-h-screen bg-[#020817]">
      ...
      <main className="relative z-10 mx-auto max-w-6xl px-6 py-16">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-white/5 bg-slate-900/40 p-4">
              ...
              <div className="space-y-2.5">
                {[
                  { step: '01', text: '基于规则引擎，从文件名 / 路径识别 50+ 技术' },
          ...
          <div ref={resultRef} className="space-y-5">
            {!result && !isLoading && (
              <div className="flex h-full min-h-[400px] items-center justify-center rounded-2xl border border-dashed border-white/10">
                <div className="text-center">
                  <div className="mb-4 text-5xl opacity-20">🔬</div>
                  <p className="text-sm text-slate-500">分析结果将在这里展示</p>
                </div>
              </div>
            )}

            {isLoading && (
              <div className="flex h-full min-h-[400px] items-center justify-center rounded-2xl border border-violet-500/20 bg-violet-500/3">
                <div className="text-center">
                  <div className="mb-4 mx-auto h-10 w-10">
                    <svg className="animate-spin text-violet-400" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                    </svg>
                  </div>
                  <p className="text-sm text-violet-300">正在解析目录树…</p>
                </div>
              </div>
            )}
            ...
            {result && !isLoading && (
              <div className="animate-float-up space-y-5">
                {/* Viz mode tabs */}
                <div className="flex gap-1.5 rounded-xl border border-white/8 bg-slate-900/60 p-1.5">
                  {VIZ_MODES.map(mode => (
                    <button
                      key={mode.id}
                    ...
                    >
                      <div className="text-base leading-none">{mode.icon}</div>
      ...
      <footer className="relative z-10 border-t border-white/5 py-8 text-center">
        <p className="text-xs text-slate-600">
          ...
          <a href="https://github.com" className="hover:text-slate-400 transition-colors">
            在 GitHub 上查看
          </a>
        </p>
      </footer>
